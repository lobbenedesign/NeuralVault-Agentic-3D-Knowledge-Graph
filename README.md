# 🏺 NeuralVault — The Sovereign Agentic Knowledge OS (v4.1.4)
> ### **"NeuralVault non è un assistente personale."**
> **È un Sovereign Cognitive Engine progettato per l'evoluzione autonoma della conoscenza e dell'architettura software sulla base degli imprinting di memoria che l'utente vuole dare alla nebula, può essere utilizzato con vari LLM tra cui GPT 5.1 e Gemini 3 PRO e tanti altri, possono essere integrati anche LLM custom e possono essere utilizzati LLM open source tramite ollama (Qwen, Llama 3.1 e tanti altri) .**
...
## 📜 Vision: The Evolutionary Singularity
Traditional RAG is a static mirror. **NeuralVault v4.1.4** is a biological lens. We have evolved beyond simple persistence into **Autonomous Evolution**. The system no longer just remembers; it audits its own knowledge gaps, reconnects broken ideas, and suggests technical optimizations to its own core architecture.

---

## 🏗️ I. 4-TIER PERSISTENT ARCHITECTURE (v4.1 Upgrade)
...
1.  **Tier L1 (Atomic RAM)**: Latency **< 1ms** (Hot-path) via **Hardware Pinning**. Real-time 60fps WebGL synchronization.
2.  **Tier L2 (AegisLogStore)**: Binary **AOBF** with "Tombstone Paradox" resolution.
3.  **Tier L3 (Contextual Archive)**: **DuckDB** + HNSW Analytical engine for hybrid search (**RRF Fusion**).
4.  **Tier L4 (Evolutionary Ledger)**: Persistent audit trail of autonomous code suggestions and implemented optimizations.

---

## ⚡ Come Funziona in 60 Secondi

1. **Ingerisci**: Trascina un PDF, incolla un URL o parla al microfono.
2. **Il Vault Elabora**: BGE-M3 converte i dati in vettori 1024D; DuckDB + HNSW li indicizzano in tempo reale.
3. **Lo Sciame Agisce**: 9 agenti autonomi (Janitor, Distiller, SkyWalker...) organizzano, potano e connettono la conoscenza.
4. **Interroga**: "Spiega il bug nel commit di ieri" → RRF Fusion + TinyBERT Reranking → risposta contestuale definitiva.
5. **Il Sistema Evolve**: L'Evolution Advisor analizza i pattern d'uso e applica patch al proprio codice per ottimizzare le performance.
...
```bash
# Requisiti: Python 3.11+, Mac M1+ (consigliato)
git clone https://github.com/giuseppelobbene/neuralvault
cd neuralvault
...
# Avvio Sovereign Engine
python3 api.py

# Apri la Nebula Dashboard
# URL: http://127.0.0.1:8001
```

---

## 🐝 II. THE KINETIC SWARM: EVOLUTIONARY MISSIONS
The 9 agents are now smarter, faster, and more visual.

*   **🛡️ SE-007 (Sentinel)**: The integrity guard. Validates cryptographic signatures of nodes, reducing semantic false positives via **RRF Consensus**. Features **Neural Overload** visual feedback.
...
## 🧬 III. SOVEREIGN EVOLUTION ADVISOR [CORE v4.0]
NeuralVault is now aware of its own source code.
*   **Proactive Technical Audit**: The system analyzes its own components (`api.py`, `neural_lab.py`) and identifies bugs or optimizations.
*   **Evolution Mode Toggle**: Switch between **Research Mode** (safe ingestion) and **Evolution Mode** (autonomous growth and bridging).
*   **Collective Wisdom**: Learns from user feedback to refine its internal decision logic.
...
| Feature | Pinecone / Zilliz | Mem0 | **NeuralVault v4.1.4** |
| :--- | :---: | :---: | :---: |
...
| **Node Sprouting** | No | No | **✅ SN-008 Weaver** |
| **Self-Optimization**| No | No | **✅ Evolution Advisor** |

**Benchmark** (Mac M1/M3, 10K nodi):
- Query latency p50: **45ms** | p99: **180ms**
- Ingestion throughput: **~120 docs/min**
- Memory footprint: **~340MB base**
...
While v4.1.4 finalized evolution and autonomy, the next cycle will introduce:
...
## 👤 About the Author
**Giuseppe Lobbene** — Software architect and builder driven by the need to innovate. I spearheaded the technical growth of a beach-booking startup, scaling revenue by **10x in a single year**. **NeuralVault is my manifesto**: proof that even at night, after a full day's work, it is possible to build the future of the AI revolution. I am looking for challenges that match my hunger for innovation, to prove to myself and my son **Oliver** that talent and dedication can still change the world.

---
---

# 🏺 NeuralVault — The Sovereign Agentic Knowledge OS (v4.1.4) [ITA]

## 📜 Visione: La Singolarità Evolutiva
Il RAG tradizionale è uno specchio statico. **NeuralVault v4.1.4** è una lente biologica. Siamo andati oltre la semplice persistenza entrando nell'era dell'**Evoluzione Autonoma**. Il sistema non si limita a ricordare; individua i propri gap di conoscenza, riconnette idee frammentate e suggerisce ottimizzazioni tecniche alla sua stessa architettura core.

---

## 🏗️ I. ARCHITETTURA A 4 LIVELLI (v4.1 Upgrade)
...
*   **🛡️ SE-007 (Sentinella)**: Il garante dell'integrità. Valida le firme crittografiche dei nodi, riducendo i falsi positivi semantici tramite **Consenso RRF**. Fornisce feedback visivo via **Neural Overload**.
...
*   **✨ SY-009 (Synth)**: Genera **Scintille Creative** durante il "Neural Dreaming", proponendo connessioni emergenti tra concetti distanti.
*   **⚕️ RP-001 (Reaper)**: Il Chirurgo. Esegue la compattazione binaria fisica dello storage, recuperando MB di spazio frammentato con precisione millimetrica.
*   **🔗 CB-003 (Bridger)**: L'Architetto. Sincronizza il codice locale (AST) con la documentazione teorica, creando ponti latenti nel repository.

---

## 🧬 III. SOVEREIGN EVOLUTION ADVISOR [CORE v4.0]
NeuralVault è ora consapevole del proprio codice sorgente.
...
## 👤 Chi sono
**Giuseppe Lobbene** — Software architect e costruttore. **NeuralVault è il mio manifesto**: la prova che il talento e la dedizione possono sfidare lo status-quo. Costruito per mio figlio **Oliver**, per dimostrargli che il futuro appartiene a chi ha il coraggio di costruire un'intelligenza sovrana.
